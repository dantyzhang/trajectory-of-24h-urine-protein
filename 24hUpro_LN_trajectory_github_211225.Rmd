...
names(xydat)
xybasdat<-xydat[,c(1,3,4,7,8,9,12)]
...
vars<-c("gender", "age", "age35", 
        ...
        "fclass4","currentIS2nd", "montoshift",
...
"IorII", "III", "IV", "mix","V", "TMA", "TIsclerosis","debri", "wireloop", "necrosis", "HTN","acl", "SLEDAI12","response", "noresponse", "esrd", "die", "relapse","nopreIScurrentIS","IScurrentpre", "CTX","MMF","response_all","fclass4","currentIS2nd",
"res_wash","cr10", "cr15", "cr20", "cr25", "cr30", "cr40", "cr50" )
...
# "Cr_first_mon", "Cr_first", "Cr_last_mon", "Cr_last", "Upro_last_mon", 
# "Upro_last", "pro_dec_perc", "mon", "cr_increase", "res_mon", 
# "res_wash", "CRmon", "PRmon", "NRmon", "cr10", "cr15", "cr20",  "montoshift"
# "cr25", "cr30", "cr40", "cr50"
...
xybasdat<-xybasdat[,c("ID", "age", "gender", "sleduration", "LN_class", "Alb", "UP24H",
"eGFR_first", "eGFR_first_mon", "eGFR_last", "eGFR_last_mon",
"Cr_first_mon", "Cr_first", "Cr_last_mon", "Cr_last", "Upro_last_mon",
"Upro_last", "pro_dec_perc", "mon", "cr_increase", "res_mon",
"res_wash", "CRmon", "PRmon", "NRmon", "cr10", "cr15", "cr20",
...
nybasdat<-nybasdat[,c("ID", "age", "gender", "sleduration", "LN_class", "Alb", "UP24H","eGFR_first", "eGFR_first_mon", "eGFR_last", "eGFR_last_mon",
"Cr_first_mon", "Cr_first", "Cr_last_mon", "Cr_last", "Upro_last_mon",
"Upro_last", "pro_dec_perc", "mon", "cr_increase", "res_mon",
"res_wash", "CRmon", "PRmon", "NRmon", "cr10", "cr15", "cr20",
"cr25", "cr30", "cr40", "cr50","fclass4","center")]
...
"res_wash", "CRmon", "PRmon", "NRmon", "cr10", "cr15", "cr20",
"cr25", "cr30", "cr40", "cr50", "fclass4", "center")
factorvars<-c("gender", "LN_class", "res_wash", "cr10", "cr15", "cr20",
"cr25", "cr30", "cr40", "cr50", "fclass4", "center")
...
train<-trainData
test<-testData
svm<-svm(train[,c(2:7,9:22)],train[,1],type="C-classification",cost=10,kernel="radial",probability=TRUE,scale=FALSE)
test<-test[,colnames(train)]
pred<-predict(svm,test[,c(2:7,9:22)],decision.values=TRUE)
table(pred,test[,1])
(47)/(47+17) #0.73
...
# https://bbs.pinggu.org/thread-3229770-1-1.html
...
###exclude ER_LB and then compare  ER_LB with LR+NR=IR
```{r merge_ER_LBvsOTHERS_variabels with p_data_pre}
load("./figures/refine_beforenormal.Rdata")
load("bas.Rdata")
...
basdatatree_out<-basdatatree_out[!basdatatree_out$fclass4=="0ER_LB",] #137
...
excel<-read.csv("./figures/v3_2vs34R_nor.csv")
write.xlsx(excel,"./figures/v3_2vs34R_nor.xlsx")
```


```{r_separate data to a traing set and a testing set}
str(muldata1)
set.seed(1988) #1984 2019 1984四临床因素
...
as_train<-trainData
trainData$fclass4<-as.factor(trainData$fclass4)
fit.full.step<-glm(fclass4 ~gender+newonsetLN+sclerosis,data=trainData,family = binomial(link="logit"))#model1 0.755 0.665
...
```{r_data_pre}
dput(names(basdatatree_out))
muldata1<-basdatatree_out[,c("age","gender","sleduration",  "LNduration", "newonsetLN", "AKI", "LNCLASS_si","IorII", "III", "IV", "mix", "V","AI", "debri", "cellcrescent", "sclerosis", "TIshrink", "TIsclerosis","UP24H", "ALB", "RBC", "CR", "BNP", "CRP", "IGG", "HTN","priois")] ##去掉激素量

vars<-c("age","gender","sleduration",  "LNduration", "newonsetLN", "AKI", "IorII", "III", "IV", "mix", "V","AI", "debri", "cellcrescent", "sclerosis", "TIshrink", "TIsclerosis","UP24H", "ALB", "RBC", "CR", "BNP", "CRP", "IGG", "HTN","priois")
...
library(clustMD)
# ?clustMD
muldata1<-muldata1[,c(numvars,factorvars)] #15+12

   
library(naniar)
vis_miss(muldata1)
muldata1[factorvars]<-lapply(muldata1[factorvars],as.numeric)


    # Order variables (Continuous, ordinal, nominal)
    Y <- as.matrix(muldata1)
    ...
    ## Not run: 
    res <- clustMD(X = Y, G = 4, CnsIndx = 15, OrdIndx = 15, Nnorms = 20000,
...
muldata2<-muldata1
muldata2$id<-c(1:194)
muldata2$rown<-paste(muldata2$fclass4,muldata2$id)
rownames(muldata2)<-muldata2$rown
muldata2<-muldata2[,-c(28:30)]
mjca(muldata2)
plot(mjca(muldata2))
plot(mjca(muldata2), mass = TRUE, contrib = "absolute",
