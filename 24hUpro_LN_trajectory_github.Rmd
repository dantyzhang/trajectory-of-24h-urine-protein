...
```{r merge_NRvsOTHERS_variabels with p_data_pre}
load("./figures/refine_beforenormal.Rdata")
load("bas.Rdata")
mydata$Upro<-beforenor
class4<-bb3$pprob
...
basdatatree_out$fclass4 <-str_replace_all(basdatatree_out$fclass4 ,"2","IR")
basdatatree_out$fclass4 <-str_replace_all(basdatatree_out$fclass4 ,"1","ER")
basdatatree_out$fclass4 <-str_replace_all(basdatatree_out$fclass4 ,"3","ER")
basdatatree_out$fclass4 <-str_replace_all(basdatatree_out$fclass4,"4","IR")
basdatatree_out$fclass4<-as.factor(basdatatree_out$fclass4)

basdatatree_out$fclass4 <- relevel(basdatatree_out$fclass4, ref = "ER") 
basdatatree_out$LNCLASS <- relevel(basdatatree_out$LNCLASS, ref = "mix")
```


```{r merge_NR_variabels with p_data_pre}
dput(names(basdatatree_out))
muldata1<-basdatatree_out[,c("age","gender","sleduration",  "LNduration", "newonsetLN", "AKI", "LNCLASS_si", "AI", "debri", "cellcrescent", "sclerosis", "TIshrink", "TIsclerosis","UP24H", "ALB", "RBC", "CR", "BNP", "DD", "CRP", "IGG", "HTN",  "priois", "SLEDAI","fclass4")] ##去掉激素量

vars<-c("age","gender","sleduration",  "LNduration", "newonsetLN", "AKI", "LNCLASS_si",
 "AI", "debri", "cellcrescent", "sclerosis", "TIshrink", "TIsclerosis","UP24H", "ALB", "RBC", "CR", "BNP", "DD", "CRP", "IGG", "HTN", "priois","SLEDAI","fclass4"
)

numvars<-c("age","sleduration", "LNduration", "AI",  "cellcrescent", "sclerosis", "TIshrink", "UP24H", "ALB", "RBC", "CR", "BNP", "DD", "CRP", "IGG","Pred", "priois", "SLEDAI"
)
...
table1 <- print(table, smd = TRUE)
detach(a)
write.csv(table1, file = "./figures/v3_ERvsOTHER.csv")
excel<-read.csv("./figures/v3_ERvsOTHER.csv")
write.xlsx(excel,"./figures/v3_ERvsOTHER.xlsx")
...
table1 <- print(table, smd = TRUE)
detach(a)
write.csv(table1, file = "./figures/v3_ERvsOTHER_nor.csv")
excel<-read.csv("./figures/v3_ERvsOTHER_nor.csv")
write.xlsx(excel,"./figures/v3_ERvsOTHER_nor.xlsx")
```


```{r_merge group_decision tree}
str(muldata1)
set.seed(123)
ind <- sample(2, nrow(muldata1), replace=TRUE, prob=c(0.7, 0.3))
trainData <- muldata1[ind==1,]
...
control<-ctree_control(mincriterion= 0.90)
m2_ctree<-ctree(muldata1$fclass4 ~., data =                     
                muldata1,controls = control)
plot(m2_ctree)
...
```{r regression planA_merge41,43_42,44}
library(glmnet)
# data2<-data[!is.na(data$age),]
# data2<-data2[!is.na(data2$newonsetLN),]
# data2<-data2[!is.na(data2$TIshrink),]
# data2<-data2[!is.na(data2$AKI),]
# data2<-data2[!is.na(data2$sclerosis),]
# data2<-data2[!is.na(data2$DD),]
# data2<-data2[!is.na(data2$CR),]
# data2<-data2[!is.na(data2$BNP),]
# data2<-data2[!is.na(data2$CRP),]
# data2<-data2[!is.na(data2$priois),]
str(muldata1)

muldata1<-muldata1[,c("age","gender","sleduration",  "LNduration", "newonsetLN", "AKI", "LNCLASS_si",
 "AI",  "cellcrescent",  "TIshrink", "UP24H", "ALB",  "CR", "BNP", "DD", "IGG", "HTN", "priois","fclass4")]
...
# age+gender+sleduration+LNduration+AKI+LNCLASS_si+AI+debri+cellcrescent+sclerosis+TIshrink+TIsclerosis+ALB+RBC+CR
fit.full.step<-glm(fclass4 ~.,
                   data=trainData,family = binomial(link="logit"))
summary(fit.full.step)
...
fit.full.step<-glm(fclass4 ~
                   age + gender + LNCLASS_si + 
    cellcrescent + CR + priois,
                   data=trainData,family = binomial(link="logit"))
summary(fit.full.step)
...
# data1<-data1[!is.na(data1$newonsetLN),]
# data1<-data1[!is.na(data1$TIshrink),]
# # data1<-data1[!is.na(data1$BNP),]
# data1<-data1[!is.na(data1$CRP),]
...
roc1n <- roc(fclass4~prob, data = data1,smooth=F)
plot(roc1n, print.auc=TRUE, print.thres=TRUE,main = "ROC CURVE trainbio",
     col="#0072B5FF",print.thres.col="#BC3C29FF",identity.col="#0072B5FF",identity.lty=1,identity.lwd=1)
ci.auc(roc1n) 
# 0.814
# 95% CI:  0.6662-0.8141 (DeLong)
...
basdatatree_out$fclass4 <-str_replace_all(basdatatree_out$fclass4 ,"2","R")
basdatatree_out$fclass4 <-str_replace_all(basdatatree_out$fclass4 ,"1","R")
basdatatree_out$fclass4 <-str_replace_all(basdatatree_out$fclass4 ,"3","R")
basdatatree_out$fclass4 <-str_replace_all(basdatatree_out$fclass4,"4","NR")
basdatatree_out$fclass4<-as.factor(basdatatree_out$fclass4)

basdatatree_out$fclass4 <- relevel(basdatatree_out$fclass4, ref = "R") 
basdatatree_out$LNCLASS <- relevel(basdatatree_out$LNCLASS, ref = "mix")
```
...
table1 <- print(table, smd = TRUE)
detach(a)
write.csv(table1, file = "./figures/v3_NRvsOTHER.csv")
excel<-read.csv("./figures/v3_NRvsOTHER.csv")
write.xlsx(excel,"./figures/v3_NRvsOTHER.xlsx")
...
table1 <- print(table, smd = TRUE)
detach(a)
write.csv(table1, file = "./figures/v3_NRvsOTHER_nor.csv")
excel<-read.csv("./figures/v3_NRvsOTHER_nor.csv")
write.xlsx(excel,"./figures/v3_NRvsOTHER_nor.xlsx")
```
